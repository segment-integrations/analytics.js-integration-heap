...
Heap.prototype.initialize = function() {
  ...
  window.heap.load = function(appid, config) {
    ...
    each(heapMethods, function(method) {
      ...
      console.log(window.heap[method]);
    });

      console.log(window.heap);
    // Because our .ready() function may not execute in time after the Heap library is loaded(),
    // customers like Casper are missing event properties on the initial pageview tracked by Heap
    var props = window.heapEventProperties || {};
    if (is.object(props) && !is.empty(props) && window.heap && window.heap.addEventProperties) {
